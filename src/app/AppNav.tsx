...
const ITEMS: NavItem[] = [
    ...
    },
    {
        ...
        labelKey: 'nav.chats',
        icon: <Icon
            d="M21 11.5a8.38 8.38 0 0 1-8.5 8.5 9 9 0 0 1-4-.9L3 21l1.9-5.5A8.38 8.38 0 0 1 4 11.5 8.5 8.5 0 0 1 12.5 3 8.5 8.5 0 0 1 21 11.5z"/>
    },
    {
...
export function AppNav({variant}: { variant: 'rail' | 'bar' }) {
    ...
    if (variant === 'bar') {
        return (
            <nav
                className="fixed inset-x-0 bottom-0 z-20 flex items-stretch border-t border-line/10 bg-void/50 backdrop-blur-xl lg:hidden">
                {ITEMS.map((item) => {
    ...
    return (
        <nav
            className="hidden h-full w-[88px] shrink-0 flex-col items-center gap-2 border-r border-line/10 bg-void/25 py-6 backdrop-blur-xl lg:flex xl:w-[228px] xl:items-stretch xl:px-4">
            <div className="mb-6 flex items-center gap-2.5 px-1 xl:px-2">
                <svg width="28" height="28" viewBox="0 0 24 24" className="shrink-0 text-accent" fill="currentColor"
                     aria-hidden>
                    <path
                        d="M12 21s-7.5-4.6-10-9.2C.5 8.6 2 5.5 5 5.5c1.9 0 3.2 1.1 4 2.3.8-1.2 2.1-2.3 4-2.3 3 0 4.5 3.1 3 6.3C19.5 16.4 12 21 12 21z"/>
                </svg>
                <span
                    className="hidden font-display text-lg font-extrabold tracking-tight text-strong xl:inline">AetherLove</span>
            </div>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isActive ? 'bg-accent/15 text-accent-light' : 'text-subtle hover:bg-surface/5 hover:text-strong',
                            )}
                        >
                            {isActive && <span
                                className="absolute left-0 top-1/2 h-6 -translate-y-1/2 rounded-r-full bg-accent-light xl:w-1"/>}
                            <span className="relative">
                {item.icon}
                                {item.screen === Screen.ChatList && newMatches > 0 && (
                                    <span
                                        className="absolute -right-1 -top-1 h-2.5 w-2.5 rounded-full bg-danger ring-2 ring-void/40"/>
                                )}
              </span>
