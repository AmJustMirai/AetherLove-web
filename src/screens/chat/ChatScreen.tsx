    ...
    return (
        <div className="mx-auto flex h-full w-full max-w-2xl flex-col">
            <header className="flex items-center gap-3 border-b border-line/10 px-4 py-4">
                <button onClick={() => router.navigate(Screen.ChatList)}
                    ...
                    <>
                        ...
                        {chat.messages.map((m) => (
                            <div key={m.id} className={cn('flex', m.fromMe ? 'justify-end' : 'justify-start')}>
                                <div
                                    className={cn(
                                        'max-w-[78%] rounded-2xl px-3.5 py-2 text-[14px]',
                                        m.fromMe ? 'bg-accent text-strong' : 'bg-surface/10 text-body',
                                    )}
                                >
                                    {m.text}
                                    {m.fromMe && m.read && <span
                                        className="ml-2 align-baseline text-[10px] text-strong/70">{t('chat.read')}</span>}
                                </div>
                            </div>
                        ))}
                    </>
